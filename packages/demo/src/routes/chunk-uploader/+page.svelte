<script lang="ts">
import { type ChunkFileEvent, ChunkUploader } from '@liwe3/webcomponents-svelte';
...
let recentFiles = $state<ChunkFileEvent[]>( [] );
...
const normalizedChunkSize = $derived.by<number>( () => Math.max( 1, Number( chunkSize ) || 1 ) );

const normalizedMaxFileSize = $derived.by<number>( () => Math.max( 1, Number( maxFileSize ) || 1 ) );
...
const handleFileComplete = ( event : ChunkFileEvent ) => {
	recentFiles = [
		event,
		...recentFiles.filter( ( item ) => item.id !== event.id ),
	].slice( 0, 5 );
	pushLogEntry( `✅ ${event.originalFileName} finished uploading` );
};

const handleUploadComplete = ( events : ChunkFileEvent[] ) => {
	// recentFiles = event.slice(-5).reverse();
	pushLogEntry(
		`🎉 Uploaded ${events.length} file${events.length === 1 ? '' : 's'}`,
	);
};

const handleUploadAborted = ( events : ChunkFileEvent[] ) => {
	pushLogEntry(
		`⚠️ Aborted ${events.length} file${events.length === 1 ? '' : 's'}`,
	);
};
</script>
...
<div class="page-container">
	<header class="page-header">
		<div>
			...
			<p class="subtitle">
				Upload multi-gigabyte files through the Cloudflare Worker in <code>servers/upload-demo</code>.
			</p>
		</div>
	</header>
	...
	<section class="callout">
		...
		<ol>
			...
			<li>
				Keep wrangler running on <code>http://127.0.0.1:8787</code> or update the URL below.
			</li>
		</ol>
	</section>
