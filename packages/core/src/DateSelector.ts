...
export class DateSelectorElement extends HTMLElement {
	...
	private generateCalendarGrid ( year : number, month : number ) : string {
		...
		for ( let i = firstDayOfWeek - 1; i >= 0; i-- ) {
			...
			grid += `<div class="day-cell other-month" data-date="${prevYear}-${( prevMonth + 1 ).toString().padStart( 2, '0' )}-${
		...
		for ( let day = 1; day <= remainingCells; day++ ) {
			grid += `<div class="day-cell other-month" data-date="${nextYear}-${( nextMonth + 1 ).toString().padStart( 2, '0' )}-${
				day.toString().padStart( 2, '0' )
			}">${day}</div>`;
		}
