class l extends HTMLElement {
	...
	show ( t ) {
		this.config = { ...this.config, ...t },
			t.buttons && t.buttons.length > 0 && ( this.config.duration = 0 ),
			t.title && t.title.trim() !== '' ? this.title = t.title : ( this.removeAttribute( 'title' ), this.config.title = '' ),
			this.text = t.text,
			t.type && ( this.type = t.type ),
			t.icon !== void 0 && ( this.icon = t.icon ),
			t.closable !== void 0 && ( this.closable = t.closable ),
			t.buttons && t.buttons.length > 0 ? this.duration = 0 : t.duration !== void 0 && ( this.duration = t.duration ),
			t.buttons && ( this.buttons = t.buttons ),
			this.render(),
	...
	close () {
		...
		} else this.dispatchEvent( new CustomEvent( 'close' ) ), this.config.onclose && this.config.onclose(), this.remove();
	}
	/**
	...
	startAutoCloseTimer () {
		this.clearAutoCloseTimer(),
			this.duration > 0 && ( this.remainingTime = this.duration,
	...
	pauseAutoCloseTimer () {
		this.autoCloseTimer
			&& this.duration > 0
			&& ( clearTimeout( this.autoCloseTimer ), this.autoCloseTimer = void 0, this.remainingTime -= Date.now() - this.pauseTime, this.pauseProgressBarAnimation() );
	}
	/**
	 * Resumes the auto-close timer
	 */
	resumeAutoCloseTimer () {
		!this.autoCloseTimer && this.remainingTime > 0 && ( this.pauseTime = Date.now(),
			this.autoCloseTimer = window.setTimeout( () => {
				this.close();
			}, this.remainingTime ),
	...
	startProgressBarAnimation () {
		!this.progressBar
			|| this.duration <= 0
			|| ( this.progressBar.style.animation = 'none', this.progressBar.offsetWidth, this.progressBar.style.animation = `shrinkProgress ${this.duration}ms linear forwards` );
	}
	/**
	...
	resumeProgressBarAnimation () {
		...
		const e = window.getComputedStyle( this.progressBar ).width,
			o = this.progressBar.parentElement?.offsetWidth || 1,
			i = parseFloat( e ) / o * 100,
			r = this.remainingTime,
			a = `shrinkProgress-${Date.now()}`,
			n = this.shadowRoot.styleSheets[0],
			c = `
      @keyframes ${a} {
        from {
          width: ${i}%;
	...
	bindEvents () {
		this.shadowRoot.addEventListener( 'click', ( e ) => {
			const o = e.target;
			if ( o.closest( '.close-button' ) ) this.close();
	 ...
	 */
	render () {
		const t = this.getTypeColors(),
			e = this.icon ? `<img src="${this.icon}" alt="Toast icon" class="toast-icon-img" />` : `<span class="toast-icon-default">${this.getDefaultIcon()}</span>`;
    ...
    `,
			this.progressBar = this.shadowRoot.querySelector( '.toast-progress-bar' ),
			this.bindEvents();
	}
}
	...
	m = ( s = 'TR' ) => {
		...
		if ( !e ) {
			...
			o.top && ( e.style.top = o.top ),
				o.bottom && ( e.style.bottom = o.bottom ),
				o.left && ( e.style.left = o.left ),
				o.right && ( e.style.right = o.right ),
				e.style.alignItems = o.alignItems,
				( s === 'T' || s === 'B' ) && ( e.style.transform = 'translateX(-50%)' );
			const i = `${t}-styles`;
